...
#define SPI_ANALOG_OUTPUTS_CH   SPI_SLAVE_3  //slave 3
...
#else
//static UINT16 Aout_tx1[14] = {0x7FF, 0x7FF, 0x7FF, 0x7FF, 0x7FF, 0x7FF, 0x7FF, 0x7FF, 0x7FF, 0x7FF, 0x7FF, 0x7FF, 0x7FF, 0x7FF}; 
static UINT16 Aout_tx1[5] = {0xFFF, 0xFFF, 0xFFF, 0xFFF, 0xFFF}; 
#endif
...
/**************************************************************************//**
* \brief   - This function is responsible for transmitting ADC 7689 configuration setting.
*
* \return  - UINT16
*
*****************************************************************************/
static UINT16 AnlgSetADC7689Config()
    {
    union ADC_CONFIG Adc_config;
    Adc_config.ADC_CFG = 0;
    Adc_config.ADC_Register.CFG  = Overwrite;
    Adc_config.ADC_Register.INCC = UniRef2GND;
    Adc_config.ADC_Register.INx  = CH7;
    Adc_config.ADC_Register.BW   = Full;
    Adc_config.ADC_Register.REF  = ExtRef;
    Adc_config.ADC_Register.SEQ  = ScanChannel;
    Adc_config.ADC_Register.RB   = NoRead;
    return Adc_config.ADC_CFG;
    }
...
static STATUS ScLoopDA(UINT16 channel)
    ...
    while(cnt < NELEMENTS(Aout_tx1))
        ...
        if(vxbMcSpiTrans(SPI_PORT_1, &hw, &pkt) != OK)
            {
            printf("failed to transfer data for CH%d D/A Output\n", channel);
            return ERROR;
            }
            ...
            switch(channel)
                ...
                case DAC_CH_A:
                case DAC_CH_B:
                    printf("CH:%d    D/A Output: 0x%x  (%4dmV)\n", channel, Aout_tx1[cnt], Aout_tx1[cnt]*2048/4096*2);
                    break;
                case DAC_CH_C:
                ...
                case DAC_CH_D:
                    ...
                    break;
                default:
                    printf("CH:%d    D/A Output: 0x%x  (%4dmV)", channel, Aout_tx1[cnt], Aout_tx1[cnt]*2048/4096*2);
                    break;
                }
            }
        taskDelay(spiDelay);
...
/**************************************************************************//**
* \brief   - This function is responsible for SPI-3 A/D 8 channel loop test
*
* \param   - None
*     
* \return  - UINT16
...
static STATUS ScLoopAD()
    ...
    hw.mode       = SPI_MODE_3 | SPI_FULL_DUPLEX;
    hw.dataLines  = 1;
    SpiAout_txBuf = AnlgSetADC7689Config();
    printf("Configuration Data: 0x%x \n", SpiAout_txBuf);
    ...
    if(vxbMcSpiTrans(SPI_PORT_1, &hw, &pkt) != OK)
        {
        printf("failed to transfer data for A/D Configuration\n");
        return ERROR;
        }
    else
        {
        memcpy(&SpiAin_rxBuf, pkt.rxBuf, sizeof(UINT16));
        printf("CH n-2 : 0x%x\n", SpiAin_rxBuf);
        }
    
    SpiAout_txBuf = 0;
    hw.mode       = SPI_MODE_2 | SPI_FULL_DUPLEX;
    if(vxbMcSpiTrans(SPI_PORT_1, &hw, &pkt) != OK)
        {
        printf("failed to transfer data for A/D Reading\n");
        return ERROR;
        }
    else
        {
    ...
    for(cnt = 0; cnt < 7; cnt++)
        ...
        if(vxbMcSpiTrans(SPI_PORT_1, &hw, &pkt) != OK)
            {
            printf("failed to transfer data for CH%d A/D Reading\n", cnt);
            return ERROR;
            }
        else
            {
            ...
            switch(cnt)
                {
                case CH0:
                    printf("CH:%d    A/D Input:  0x%x  (%5dmv)\n", cnt, SpiAin_rxBuf, SpiAin_rxBuf*20000/65535 - 10000);
                    break;
                case CH1:
                    printf("CH:%d    A/D Input:  0x%x  (%5dmv)\n", cnt, SpiAin_rxBuf, SpiAin_rxBuf*10000/65535);
                    break;
                case CH2:
                    printf("CH:%d    A/D Input:  0x%x  (%5dmv)\n", cnt, SpiAin_rxBuf, SpiAin_rxBuf*10000/49151);
                    break;
                case CH3:
                    printf("CH:%d    A/D Input:  0x%x  (%5dmv)\n", cnt, SpiAin_rxBuf, SpiAin_rxBuf*20000/65535 - 10000);
                    break;
                case CH4:
                    ...
                    break;
                case CH5:
                    printf("CH:%d    A/D Input:  0x%x  (%5dmv)\n", cnt, SpiAin_rxBuf, SpiAin_rxBuf*5000/65535);
                    break;
                case CH6:
                    printf("CH:%d    A/D Input:  0x%x  (%5dmv)\n", cnt, SpiAin_rxBuf, SpiAin_rxBuf*5000/65535);
                    break;
                case CH7:
                    ...
                    break;
                default:
                    printf("CH:%d is out of Channel forA/D Reading\n", cnt);
                    break;
                }
            }
        }
...
void testSpi3()
    ...
    printf("\n* SPI-3 Channel-3  LOOP D/A TEST: *\n"); 
    ScLoopDA(DAC_CH_D);
    taskDelay(spiDelay);
    printf("\n* SPI-3 LOOP A/D 8 Channels TEST: *\n"); 
    for(retried = 0; retried < 1; retried++)
        {
        printf("\n* #%d SPI-3 LOOP A/D 8 Channels TEST: *\n", retried); 
        ScLoopAD();
        taskDelay(spiDelay);
        }
//    printf("\n* SPI-3 Channel-0  LOOP D/A TEST: *\n");  
...
void testSpi1()
{
	while(1)
	{
        printf("\n* SPI-1 LOOP A/D 8 Channels TEST: *\n"); 
        ScLoopAD();
        taskDelay(spiDelay);
	}
}
