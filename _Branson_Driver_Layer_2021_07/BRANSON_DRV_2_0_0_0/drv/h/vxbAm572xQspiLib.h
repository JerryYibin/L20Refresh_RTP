...
typedef enum qspiLibCmd
{
    ...
    /**< Quad read commnad from QSPI flash */
	 QSPI_LIB_CMD_WRITE_DISABLE = 0x4U,
	 /**< QSPI flash write disable command */
    ...
    QSPI_LIB_CMD_READ_FRAM_MFG_ID = 0x9FU,
    /**< Read FRAM manufacturer and device ID */
...
void vxbQSPIRegister   (	TI_AM437X_SPI_CTRL *    pDrvCtrl  );

void vxbQSPILibInit(void);
void vxbQSPILibFramInit(void);

void vxbQSPILibBlockErase(uint32_t blockNumber);

void vxbQSPILibChipErase(void);

void vxbQSPILibPrintId(void);
void vxbQSPILibFramPrintId(void);

int32_t vxbQSPILibRead(
                 uint32_t srcOffsetAddr,
                 uint32_t dstAddr,
                 uint32_t length);
int32_t vxbQSPILibFramRead(
                 uint32_t srcOffsetAddr,
                 uint32_t dstAddr,
                 uint32_t length);

int32_t vxbQSPILibWrite(
                  uint32_t dstOffsetAddr,
                  uint32_t srcAddr,
                  uint32_t length);
int32_t vxbQSPILibFramWrite(
                  uint32_t dstOffsetAddr,
                  uint32_t srcAddr,
                  uint32_t length);

void vxbQSPILibQuadEnable(qspiFlashDeviceData_t *qspiFlashDeviceData);
