/* vxbTiAm3Sio.c - TI AM3xxx SIO Driver */
...
--------------------
11nov16,jnl  fixed spin lock issues in smp (F7314)
19aug16,jnl  set tx and rx trigger bit to reduce SIO interrupts (V7PRO-3031)
17may16,sye  get default baudrate from DTS if existed (V7PRO-2923)
14nov15,yjw  update for am57xx SoC (US64520)
26oct15,g_x  add hardware auto-CTS/auto-RTS flow control (V7PRO-2313)
05aug14,swu  fix build warning. (V7PRO-1081)
25jun14,c_t  written from vxbNs16550.c.(US40510)
*/
...
#define AM3_SIO_MIN_RATE 50
#define AM3_SIO_MAX_RATE 115200
...
LOCAL STATUS tiAm3SioAttach
    ...
    if ((prop = (UINT32 *)vxFdtPropGet (pFdtDev->offset,
        ...
        {
        pChan->txTrgLvl = vxFdt32ToCpu (prop[0]);
        }
    else
        {
    ...
    if ((prop = (UINT32 *)vxFdtPropGet (pFdtDev->offset,
        ...
        {
        pChan->rxTrgLvl = vxFdt32ToCpu (prop[0]);
        }
    else
        {
    ...
    if (prop == NULL)
        ...
        pChan->fifoSize = vxFdt32ToCpu (prop[0]);
...
LOCAL STATUS tiAm3SioCreate
    ...
    (void) snprintf (pChan->tyName, 20, "%s%d", "/tyCo/", pChan->channelNo);

    if (ttyDevCreate (pChan->tyName, (SIO_CHAN *)pChan,
                      VXB_SIO_TX_BUF_SIZE, VXB_SIO_TX_BUF_SIZE) != OK)
        return ERROR;
...
LOCAL void tiAm3SiovxbInitChannel
    ...
    REG_SET(TLR, pChan, pChan->txTrgLvl | (pChan->rxTrgLvl << 4)); /* 7. */
    REG_SET(LCR, pChan, 0x80); /* 10. */
    REG_SET(LCR, pChan, pChan->lcr); /* 12. */

    /* set the requested baud rate */

    (void) tiAm3SiovxbBaudSet(pChan, (UINT)pChan->baudRate);
...
LOCAL STATUS tiAm3SiovxbOptsSet
    ...
    if (!(options & CLOCAL))
        ...
        if (pChan->fifoSize >= 2)
            {
            rxTrigHlt = pChan->fifoSize - 1;
            }
        else
            {
            rxTrigHlt = 1;
            }
        REG_SET(TCR, pChan, ((rxTrigHlt - 1) << 4) | rxTrigHlt); /* 5. */
...
LOCAL STATUS tiAm3SiovxbModeSet
    ...
    if (newMode == SIO_MODE_INT)
        ...
        if ((pChan->queueId == NULL) && ((!pChan->flag & DISABLE_DEFER)))
